...
With that, we have completed our SAT solver. For something built in 9 weeks, it's 
pretty algorithmically rich. But how does it compare to MiniSAT? That is what this 
section aims to find out. In order to compare them together, we can't use Test++ 
as all MiniSAT code I can find isn't C++20, and won't compile under those standards. 
Therefore, a custom testing environment must be built. And since we're using a custom 
environment, we will also change our testing sets as well. 

I can't do MiniSAT justice in my own explanations of it. However, for those who 
don't know, MiniSAT was the winner of three categories in the 2005 SAT solving 
competition, and is considered to be one of \textbf{the} SAT solvers to use.
More about MiniSAT can be found here:
\begin{center}
    \url{http://minisat.se}
\end{center}

Because we're comparing our SAT solver to a competition-winning solver, 
I thought it would be fun and cool to use benchmarks from a SAT solver 
competition. After scouring the internet, I was able to find the SAT competition
website.
\footnote{It didn't take much scouring.}
Unfortunately, I was unable to find any of the benchmark files from the competitions. 
Before you find the website yourself and go ``it's right there moron,'' I think you 
and I both know I'm smart enough to click a preview/download link to view the files. 
However, I was met with either 404 errors or the page wouldn't load. Luckily, I was 
able to find a website that claims to have competition benchmarks from 2002 to 2024. 
Why do I say ``claims?'' I say that because the one downloadable file that appears to 
actually be the benchmarks and not file names or hashes, is 28.7GB in size. And that's 
a .zip file, so these are the \textit{compressed} benchmarks. I have spent over 8 hours 
trying to download this file and I still have over 9GB to go as of this writing.

Okay so I just clicked a different link on the website and found a site that also has 
benchmarks, with their results as well, and downloading singular files is way easier
there. I'm not sure what to make of this because I feel like I wasted the last night 
sleeping waiting for this giant file (that at this point can only be described as a .zip 
bomb for a normal person) to download. So given how that's the case, there will be two 
comparisons (maybe). The first will be against all the files that I can download quickly,
and the second (if I decide to do it) will be against the files in the giant .zip file. 

The files for the first comparison can be found here:
\begin{center}
    \url{https://benchmark-database.de}
\end{center}

MiniSAT code found here:
\begin{center}
    \url{https://github.com/niklasso/minisat}
\end{center}

I couldn't find any rules about how benchmarking is set up or the rules on how they're 
run, other than the fact that your SAT solver must produce the correct output and reasoning
for it. Because of that, we will use our own set up. For each benchmark file, we will 
make up our own rules. We'll run each benchmark file into a solver three times as a warmup, 
then we will run it 3 times as a trial, and then we'll take the average time for that file. 
Because I couldn't find a ``reset" method to reset the solver back to its starting state as 
if it just exited the constructor in MiniSAT's code, we will just reconstruct the solver 
each time.



Files found here:
\url{https://zenodo.org/records/15125952?preview_file=undefined}
