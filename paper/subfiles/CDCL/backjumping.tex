...
\begin{algorithm}[H]
\caption{CDCL with Backjumping}\label{alg:32}
\begin{algorithmic}[1]
\Procedure{solve}{}
...
\While{$true$}
    ...
    \If{succeeded is not null}
        ...
        \State maxLevel $\gets$ decisionLevel
        \State sLevel $\gets$ -1

        \For{each $var$ in $learned$}
            \State idx $\gets \Call{abs}{var}$
            \State dLvl $\gets$ trail[varToTrail[idx]].decisionLevel
            \If{dLvl $\neq$ maxLevel}
                \State sLevel $\gets \Call{max}{sLevel, dLvl}$
            \EndIf
        \EndFor

        \While{trail is not empty $\wedge$ trail.last.decisionLevel > sLevel}
            \State remove entries from the trail
            \State remove decision levels from trailStarts as necessary
        \EndWhile

        \State decisionLevel $\gets$ sLevel
        \State trailHead $\gets$ trailStarts.last
        \State $\Call{createWatched}{learned, decisionLevel}$
