...
language. In the first draft, I used a C-based syntax for readability purposes. However, what I 
thought was a readability issue was actually my lack of ability to properly format pseudocode using LaTeX. 
So in this version, we will use actual pseudocode syntax, and therefore, some proficiency with pseudocode is 
required to read this. Luckily, I will still explain what's going on, and I think you'll be 
able to pick it up quite easily. Just like in the first version, we will pretend our arrays 
and other data structures are dynamically resizable and don't require any pre-allocation of 
space.
...
That being said, you will see ``our interface'' sections every now and then when we make changes to our 
SAT solver object. The syntax in those sections will be more C++ based than pseudocode based because that 
is part of our actual implementation.
...
\begin{itemize}
    ...
    \item Testing and Benchmarking: Testing and benchmarking is done by Test++, a C++ 
            ...
            As of version 20.1.3, the program itself is compiled to be optimized, and 
            you can choose to set the optimization flags of the program you're compiling.
            This potentially can change the runtime of our program. This is important because 
            we will be using the amount of time Test++ takes to finish a test suite as the 
            time our solver took to solve each problem, even though Test++ is not technically 
            a benchmarking program. The way tests are set up is that 
...
\begin{algorithm}[H]
\caption{SAT solver constructor}\label{alg:cap}
\begin{algorithmic}[1]
\Procedure{Constructor}{$input$}
\If{\texttt{input is a file}}
    \State $content \gets \Call{readFile}{input}$
    \State $\Call{parse}{content}$
\Else \Comment{We assume input is a CNF equation}
    \State $\Call{parse}{input}$
\EndIf
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
\caption{Input parsing}\label{alg:2}
\begin{algorithmic}[1]
\Procedure{Parse}{$equation$}

\If{\texttt{$equation$ contains "p cnf"}}
    \State  $A[1..n]$
    \For{$line$ in $input$}
        \If{$line$ starts with "c"}
            \State \texttt{continue on to the next line}
        \Else
            \For{$token$ in $line$}
                \State \Call{Append}{A, token}
            \EndFor
        \EndIf
    \EndFor

    \State $numVars \gets A[1]$
    \State $numClauses \gets A[2]$

    \State $idx \gets 2$

    \For{$i \gets 1$ \texttt{up to} $numClauses$}
        \State $Clause[1..n]$
        \While{$A[i] \neq 0$}
            \Call{Append}{Clause, A[i]}
            \State $idx \gets idx + 1$
        \EndWhile
        \Call{Append}{Clauses, Clause}
    \EndFor
\Else 
    \State $numVars \gets 0$
    \State $numClauses \gets 0$
    \State $A[1..n]$

    \For{$line$ in $input$}
        \If{$line$ starts with "c"}
            \State \texttt{continue on to next line}
        \Else
            \For{$token$ in $line$}
                \State $numVars \gets \Call{max}{numVars, token}$
                \If{$token = 0$}
                    \State $numClauses \gets numClauses + 1$
                \EndIf
                \Call{Append}{A, token}
            \EndFor
        \EndIf
    ...
    \For{$i \gets 1$ \texttt{up to} $numClauses$}
        \While{$A[i] \neq 0$}
            \State $\Call{Append}{Clause, A[idx]}$
            \State $idx \gets idx + 1$
        \EndWhile
...
\footnote{What if there are multiple ``p cnf'' in the equation? They all better be commented is all 
I have to say on that matter. In my implementation I try to stop the user from shooting themselves in 
the foot if they try to shoot themselves in the foot, however, it's completely valid to just let the 
user shoot themselves in the foot and learn not to do that.}