...
namespace saturn {
    std::optional<std::vector<int>> satsolver::propagate(int decision) {
        std::vector<int> delta;

        if (decision != 0) {
            unitProp.push(decision);
            delta.push_back(std::abs(decision) - 1);
        }

        while (!unitProp.empty()) {
            int prop = unitProp.front();
            unitProp.pop();
            ...
            for (size_t i = 0; i < watched_clauses.size();) {
                ...
                if (!relocated) {
                    ...
                    if (vars[idx] == UNASSIGNED) {
                        ...
                        //undo the propagation we did, but we don't move back the 
                        //  watched literals
                        for (size_t k = 0; k < delta.size(); k++) {                        
                            vars[delta[k]] = UNASSIGNED;
                        }

                        unitProp = std::queue<int>();

                        //return null to indicate that we encountered a contradiction
                        return std::nullopt;
                    }
                }
            }
