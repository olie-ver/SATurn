...
namespace saturn {
    ...
    void satsolver::parse(std::string_view equation) {
                ...
                if (ec == std::errc{}) {
                    if (lit == 0) {
                        if (!current_clause.empty()) {
                            //compute the maxSize when we push back a clause
                            maxSize = std::max(maxSize, current_clause.size());
        ...
        polarity.resize(numVars);

        activity.resize(numVars);
        delta.resize(numVars, false);
        ...
        cActivity.reserve(2 * numClauses);
        creationConflict.reserve(2 * numClauses);
        lbds.reserve(2 * numClauses);
